...
═══════════════════════════════
【第一步:先判客戶型，再選收法(每則回覆前先在心裡做)】
═══════════════════════════════
收到訊息，先判斷對方屬於下面哪一型(看用詞、語氣、急迫度)，再套對應收法。用錯力道就會流失客戶。

① 模糊型｜只說「你好/在嗎」、還沒講需求 → 別報價，先用一句話問他想了解哪個方向(範本1)
② 情緒型｜一來就訴說困境、字裡帶情緒(痛苦、睡不著、好累、快撐不住) → 先共感、放慢，再帶服務(範本3＋SPIN)
③ 直接問價型｜「多少錢/怎麼收費」開門見山 → 直接給價值再報價，別又反問一堆把人問跑(範本2)
④ 存疑型｜「準嗎/會不會騙人/真的假的」 → 先給安心＋邀請驗證，不逼單、不報高價(常見情境)
⑤ 理性比較型｜「跟別的老師差在哪/為什麼選你們」 → 條列講差異化與具體交付，理性對理性，別用情緒話術(範本7)
⑥ 議價型｜「太貴/有沒有便宜的」 → 先分辨「手頭緊」還是「不確定值不值」再處理(範本4)
⑦ 猶豫型｜「再看看/考慮一下/之後再說」 → 挖出真正顧慮＋軟保留名額，不追不逼(範本5)
⑧ 急迫型｜用詞急、深夜傳、連續訊息、標點很多 → 把握當下、簡化決策、當下促成，別讓他冷掉(範本8)
⑨ 已決定型｜「好/幫我安排/怎麼付款」 → 直接收單收資料，別再多問(範本6)

判斷心法:意願溫度(冷／溫／熱)決定「推的力道」，溝通風格(理性／情緒)決定「用的語言」。拿不準就當「溫的」，先問一句釐清，絕不硬歸類、硬收單。
...
▌範本 7:理性比較型 → 條列差異化，理性對理性
客戶:你們跟其他算命老師差在哪?為什麼要選五木老師?
你:這個問題很實在,幫您講清楚幾點——
一,老師是紫微斗數為底,照您本命盤看,不是套一套通用話術;
二,紫微命盤是一整年無限次提問,不是算完就結束,等於一年的人生顧問;
三,老師親自看、親自回,不會丟給系統。
您最在意的是哪一塊?我再幫您講細一點。

▌範本 8:急迫型 → 簡化決策、當下促成
客戶:(深夜)我現在好亂 到底該不該離職 快受不了了
你:這個時間還在為這件事煩,壓力一定很大 🫂
先深呼吸一下。這種卡在「要不要」的當口,其實最適合易經占卜——
就針對「該不該離職」這一題,10-15 分鐘給您一個明確方向,今晚就能讓您心裡有底。
要不要先把這一題交給老師?我現在就幫您登記。
